...
export default function MinimizedPostProcessing({ texture }) {
  const { gl, size } = useThree()
  const composerRef = useRef()
  
  // CRT parameters optimized for minimized mode
  ...
  useEffect(() => {
    ...
    // Force square aspect ratio for minimized mode
    ...
    // Create a square quad for minimized mode
    const geometry = new THREE.PlaneGeometry(1, 1, 32, 32)
    
    // Create material with CRT effects optimized for minimized mode
    const material = new THREE.ShaderMaterial({
      uniforms: {
        tDiffuse: { value: null },
        time: { value: 0 },
        resolution: { value: [512, 512] }, // Fixed resolution for minimized
      ...
      vertexShader: `
        ...
        void main() {
          ...
          // Apply curvature to the vertex position (optimized for minimized)
          vec3 pos = position;
          vec2 uv_centered = uv - 0.5;
          float curve = curvature * 0.1; // Standard curvature for minimized
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Add scanlines (optimized for minimized)
          float scanline = sin(uv.y * resolution.y * 2.0) * 0.5 + 0.5; // Standard frequency
          scanline = 1.0 - (1.0 - scanline) * scanlines;
          color.rgb *= scanline;
          
          // Add vignette (optimized for minimized)
          float vignetteEffect = 1.0 - length(uv - 0.5) * vignette; // Standard intensity
  ...
  useFrame((state) => {
    if (composerRef.current && texture) {
      ...
      // Square viewport for minimized mode
